...
krsa_df <-
  read_csv("results/STR-krsa_table_full_STR_Exer_STR_CTL_PTK.csv") |>
  select(Kinase, Score = AvgZ) |>
  unique() |>
  read_krsa(trns = "abs", sort = "desc")

uka_df <- read_tsv("kinome_data/UKA-PTK/STR/Summaryresults 20240126-1027.txt") |> select(Kinase = `Kinase Name`, Score = `Median Final score`) |>
  unique() |>
  read_uka(trns = "abs", sort = "desc") |>
  unique()

diff_peps <- read_csv("results/STR-dpp_STR_Exer_STR_CTL-PTK.csv") |>
...
combined <- combine_tools(krsa_df, uka_df, kea_df, ptmsea_df) |>
  write_csv("results/STR-PTK-Combined_Tools.csv")
...
quartile_fig <- combined |>
  ...
  quartile_figure() |>
  ggsave("figures/str_ptk_quartile_figure.png", plot = _, width = 18, height = 6, units = "in", bg = "white")
