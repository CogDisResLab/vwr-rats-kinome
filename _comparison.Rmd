...
data_modeled$scaled |> 
  filter(Peptide %in% pep_passed_qc,
    Group %in% c("{{case}}", "{{ctrl}}")) |>
  write_csv("results/{{run_prefix}}-signal_{{case}}_{{ctrl}}-{{chip}}.csv")
...
sigPeps_total <- list(significant_peptides, sigPepsPerChip) %>%
  ...
  write_file(
    str_c(significant_peptides$meanLFC$`0.2`, collapse = "\n"),
    "results/{{if_else(exists('run_prefix'), str_c(run_prefix, '-'), '')}}significant_peptides_0.2_{{case}}_{{ctrl}}-{{chip}}.txt"
  )
```
